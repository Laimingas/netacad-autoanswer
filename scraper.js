...
function getCurrentMcqViewElement(mcqViewElements) {
  const visibleMcqViews = mcqViewElements
      .map((mcqView) => ({ element: mcqView, visibleArea: getVisibleArea(mcqView) }))
      .filter(({ element, visibleArea }) => visibleArea > 0 && isElementDisplayedInComposedTree(element))
...
function findMcqViewElements() {
  ...
  if (!pageView?.shadowRoot) return mcqViewElements;

  pageView.shadowRoot.querySelectorAll("article-view").forEach((articleView) => {
...
async function scrapeData(currentAttempt = 1) {
  const storedData = await chrome.storage.sync.get(["aiApiKey", "answerMode"]);
  const apiKey = storedData.aiApiKey;
  const answerMode = storedData.answerMode || "auto";
  ...
  // If mode is AI-only or auto, AI key is needed
  if ((answerMode === "ai" || answerMode === "auto") && !apiKey) return false;

  return processSingleQuestion(currentMcqViewElement, apiKey, answerMode);
}